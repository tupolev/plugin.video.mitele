...
def wait_if_xbmc_not_closing(espera):
    logger.info("mitele.service_subscription.wait_if_xbmc_not_closing "+repr(espera))

    while not xbmc.abortRequested and espera > 0:

        #logger.info("mitele.service_subscription xbmc.abortRequested="+repr(xbmc.abortRequested)+" espera="+repr(espera))
        # Cada segundo se comprueba si XBMC esta cerrándose, hasta que ha pasado el tiempo
        xbmc.sleep(1000)
        espera = espera - 1

    if espera==0:
        logger.info("mitele.service_subscription Wait finished")
...
if config.get_setting("suscription_check")=="true":
    ...
    logger.info("mitele.service_subscription First time launch")

    while not xbmc.abortRequested:

        logger.info("mitele.service_subscription Checking for new items in subscriptions")
        ...
        logger.info("mitele.service_subscription Done, waiting for the expected lapse...")
        wait_if_xbmc_not_closing(8*60*60)

    logger.info("mitele.service_subscription XBMC Abort requested")

else:

    logger.info("mitele.service_subscription Suscription check disabled in config")
