...
def play_video(item,desdefavoritos=False,desdedescargados=False,desderrordescargas=False,strmfile=False):
    logger.info('mitele.platformcode.xbmctools add_new_video item='+item.tostring())
    ...
    if strmfile: #Si es un fichero strm no hace falta el play
        ...
        #    logger.info("mitele.platformcode.xbmctools Con subtitulos")
        #    setSubtitles()
    else:
        if use_download_and_play or config.get_setting("player_mode")=="3":
            ...
            logger.info("mitele.platformcode.xbmctools playersettings="+playersettings)
        
            player_type = xbmc.PLAYER_CORE_AUTO
            if playersettings == "0":
                player_type = xbmc.PLAYER_CORE_AUTO
                logger.info("mitele.platformcode.xbmctools PLAYER_CORE_AUTO")
            elif playersettings == "1":
                player_type = xbmc.PLAYER_CORE_MPLAYER
                logger.info("mitele.platformcode.xbmctools PLAYER_CORE_MPLAYER")
            elif playersettings == "2":
                player_type = xbmc.PLAYER_CORE_DVDPLAYER
                logger.info("mitele.platformcode.xbmctools PLAYER_CORE_DVDPLAYER")
...
def add_items_to_kodi_directory(itemlist,parent_item):
    logger.info("mitele.platformcode.xbmctools add_items_to_kodi_directory")
...
def add_item_to_kodi_directory( item , itemlist , channel_provides_context_menu = False ):
    logger.info('mitele.platformcode.xbmctools add_item_to_kodi_directory item='+item.tostring())
    ...
    logger.info('mitele.platformcode.xbmctools add_item_to_kodi_directory item.is_favorite_show='+item.is_favorite_show)
...
def get_context_menu_for_item( item , channel_provides_context_menu=False ):
    #logger.info("mitele.platformcode.xbmctools get_context_menu_for_item")
...
def wait2second():
    logger.info("mitele.platformcode.xbmctools wait2second")

    import time
    contador = 0
    while xbmc.Player().isPlayingVideo()==False:
        logger.info("mitele.platformcode.xbmctools setSubtitles: Waiting 2 seconds for video to start before setting subtitles")
        time.sleep(2)
        contador = contador + 1
...
# TODO: Pasar esto a custom player
def setSubtitles():
    logger.info("mitele.platformcode.xbmctools setSubtitles")
    import time
    contador = 0
    while xbmc.Player().isPlayingVideo()==False:
        logger.info("mitele.platformcode.xbmctools setSubtitles: Waiting 2 seconds for video to start before setting subtitles")
    ...
    logger.info("mitele.platformcode.xbmctools setting subtitle file %s" % subtitlefile)
