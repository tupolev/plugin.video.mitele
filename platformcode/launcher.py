...
def start():
    ...
    """
    logger.info("mitele.platformcode.launcher start")

    # Test if all the required directories are created
    config.verify_directories_created()

def run(item=None):
    logger.info("mitele.platformcode.launcher run")
    ...
    try:
        ...
        if item.action=="selectchannel":
            ...
            if config.get_setting("updatecheck2") == "true":
                logger.info("mitele.platformcode.launcher Check for plugin updates enabled")
                ...
                logger.info("mitele.platformcode.launcher Check for plugin updates disabled")
        ...
        # Action in certain channel specified in "action" and "channel" parameters
            ...
            logger.info("mitele.platformcode.launcher channel_file=%s" % channel_file)

            channel = __import__('channels.%s' % item.channel, fromlist=["channels.%s" % item.channel])
            logger.info("mitele.platformcode.launcher running channel {0} {1}".format(channel.__name__, channel.__file__))


            # Special play action
            if item.action == "play":
                logger.info("mitele.platformcode.launcher play")

                # First checks if channel has a "play" function
                if hasattr(channel, 'play'):
                    logger.info("mitele.platformcode.launcher executing channel 'play' method")
                    ...
                    logger.info("mitele.platformcode.launcher executing core 'play' method")
                    xbmctools.play_video(item)
                ...
                logger.info("mitele.platformcode.launcher search")
                ...
                logger.info("mitele.platformcode.launcher executing channel '"+item.action+"' method")
